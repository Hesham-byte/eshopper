...
const Products001 = () => {
    ...
    useEffect(() => {
        const fetchNews = async () => {
            try {
                const response = await axios.get(import.meta.env.VITE_API_URL + '/new-products');
        ...
        <>
            <div className="container-fluid pt-5">
                ...
                {news.length === 0 ? (
                    ...
                    news.map((newProduct, index) => (
                        <div className="col-lg-3 col-md-6 col-sm-12 pb-1" key={newProduct.id}>
                            <div className="card product-item border-0 mb-4">
                                <div
                                    className="card-header product-img position-relative overflow-hidden bg-transparent border p-0">
                                    <img className="img-fluid w-100" src={newProduct.image} alt=""/>
                                </div>
                                <div className="card-body border-left border-right text-center p-0 pt-4 pb-3">
                                    <h6 className="text-truncate mb-3">{newProduct.name}</h6>
